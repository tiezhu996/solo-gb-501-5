...
```bash
python3 verify.py            # 35 项断言全部通过则退出码为 0
```

校验覆盖三部分：API 业务规则（拦截/复测/关闭/放行/输入校验）、页面真实渲染
（`verify_pages.js` 以 DOM 桩加载真实 `static/app.js`，对运行中的服务逐页断言
接口数据出现在页面上）、并发安全（8 线程并发放行同一批次仅 1 次成功；
30 轮“超限读数 × 放行”并发，逐轮断言拦截/分离二选一，无不一致状态）。
页面校验也可单独执行：
...
可重复运行的回归测试（每轮独立临时实例：随机端口 + 临时数据库，结束自动清理，
可连续多轮验证稳定性）：

```bash
python3 regression_test.py              # 运行一轮，9 个用例
python3 regression_test.py --repeat 3   # 连续运行 3 轮
```

回归用例：合格读数不生成事件 / 超限自动建事件并关联批次 / 未关闭事件拦截放行 /
原因措施 + 复测合格才能关闭 / 复测仍超限不能关闭并继续拦截 / 已放行批次不能补挂事件 /
重复放行拒绝 / 已关闭事件拒绝复测 / 并发放行仅一次成功 / 超限读数与放行并发 12 轮。
...
7. **并发安全**：放行、读数登记（含事件生成）、事件关闭/处置的“检查 + 状态迁移”均在 `BEGIN IMMEDIATE` 写事务内完成，并配合原子条件更新（`UPDATE ... WHERE status=...`）与 `busy_timeout`。并发提交同一批次放行仅一次成功，其余收到 `409 ALREADY_RELEASED`；超限读数与放行并发时，要么放行被新生成的事件拦截，要么放行先成功、事件关联不到已放行批次——两种结果都一致，绝不出现“已放行批次挂着未关闭事件”。
...
├── seed.py          # 演示数据（幂等）
├── verify.py        # 端到端校验：API 规则 + 页面渲染 + 并发（35 项断言）
├── verify_pages.js  # 页面渲染校验（DOM 桩加载真实 app.js）
├── regression_test.py  # 可重复运行的回归测试（9 用例，独立临时实例，自动清理）
